            ...
            if !open.urls.isEmpty{
                if var path = open.urls.first?.path{
                    ...
                    if manager.fileExists(atPath: path + "/Contents/Resources/" + sharedExecutableName) && manager.fileExists(atPath: path + "/Contents/SharedSupport/InstallESD.dmg") && manager.fileExists(atPath: path + "/Contents/Info.plist") {
                        ...
                        if let name = open.urls.first?.lastPathComponent{
                            msgBoxWarning("Invalid file", "The app you chose, \"\(name)\", is not usable to create macOS installers or macOS installations because it does not contain all the needed files to do that.")
                        }else{
                            msgBoxWarning("Invalid file", "The app you chose is not usable to create macOS installers or macOS installations because it does not contain all the needed files to do that.")
                        }
                    }
                }else{
